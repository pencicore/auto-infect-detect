        ...
        public void updateDiagnosis(AllDiagnosis allDiagnosis) {
                ...
                if (diagnosispersonalinfo != null) {
                        int update = diagnosisPersonalInfoMapper.update(
                                        ...
                                        new LambdaQueryWrapper<Diagnosispersonalinfo>().eq(
                                                        Diagnosispersonalinfo::getDiagnosisResultsId,
                                                        diagnosisResultsId));
                        if (update == 0) {
                ...
                if (diagnosisgeneralsymptoms != null) {
                        int update = diagnosisGeneralSymptomsMapper.update(
                                        ...
                                        new LambdaQueryWrapper<Diagnosisgeneralsymptoms>().eq(
                                                        Diagnosisgeneralsymptoms::getDiagnosisResultsId,
                                                        diagnosisResultsId));
                        if (update == 0) {
                ...
                Diagnosisrespiratorysymptoms diagnosisrespiratorysymptoms = allDiagnosis
                                .getDiagnosisrespiratorysymptoms();
                if (diagnosisrespiratorysymptoms != null) {
                        int update = diagnosisRespiratorySymptomsMapper.update(
                                        diagnosisrespiratorysymptoms,
                                        new LambdaQueryWrapper<Diagnosisrespiratorysymptoms>().eq(
                                                        Diagnosisrespiratorysymptoms::getDiagnosisResultsId,
                                                        diagnosisResultsId));
                        if (update == 0) {
                ...
                if (diagnosisdigestivesymptoms != null) {
                        int update = diagnosisDigestiveSymptomsMapper.update(
                                        ...
                                        new LambdaQueryWrapper<Diagnosisdigestivesymptoms>().eq(
                                                        Diagnosisdigestivesymptoms::getDiagnosisResultsId,
                                                        diagnosisResultsId));
                        if (update == 0) {
                ...
                Diagnosiscirculatorysymptoms diagnosiscirculatorysymptoms = allDiagnosis
                                .getDiagnosiscirculatorysymptoms();
                if (diagnosiscirculatorysymptoms != null) {
                        int update = diagnosisCirculatorySymptomsMapper.update(
                                        diagnosiscirculatorysymptoms,
                                        new LambdaQueryWrapper<Diagnosiscirculatorysymptoms>().eq(
                                                        Diagnosiscirculatorysymptoms::getDiagnosisResultsId,
                                                        diagnosisResultsId));
                        if (update == 0) {
                ...
                Diagnosisneurologicalsymptoms diagnosisneurologicalsymptoms = allDiagnosis
                                .getDiagnosisneurologicalsymptoms();
                if (diagnosisneurologicalsymptoms != null) {
                        int update = diagnosisNeurologicalSymptomsMapper.update(
                                        diagnosisneurologicalsymptoms,
                                        new LambdaQueryWrapper<Diagnosisneurologicalsymptoms>().eq(
                                                        Diagnosisneurologicalsymptoms::getDiagnosisResultsId,
                                                        diagnosisResultsId));
                        if (update == 0) {
                ...
                if (diagnosislocalsymptoms != null) {
                        int update = diagnosisLocalSymptomsMapper.update(
                                        ...
                                        new LambdaQueryWrapper<Diagnosislocalsymptoms>().eq(
                                                        Diagnosislocalsymptoms::getDiagnosisResultsId,
                                                        diagnosisResultsId));
                        if (update == 0) {
                ...
                if (diagnosisothersymptoms != null) {
                        int update = diagnosisOtherSymptomsMapper.update(
                                        ...
                                        new LambdaQueryWrapper<Diagnosisothersymptoms>().eq(
                                                        Diagnosisothersymptoms::getDiagnosisResultsId,
                                                        diagnosisResultsId));
                        if (update == 0) {
                ...
                if (diagnosiscomplications != null) {
                        int update = diagnosisComplicationsMapper.update(
                                        ...
                                        new LambdaQueryWrapper<Diagnosiscomplications>().eq(
                                                        Diagnosiscomplications::getDiagnosisResultsId,
                                                        diagnosisResultsId));
                        if (update == 0) {
                ...
                if (diagnosisexaminations != null) {
                        int update = diagnosisexaminationsMapper.update(
                                        ...
                                        new LambdaQueryWrapper<Diagnosisexaminations>().eq(
                                                        Diagnosisexaminations::getDiagnosisResultsId,
                                                        diagnosisResultsId));
                        if (update == 0) {
                ...
                if (allDiagnosis.getFileIdList() != null) {
                        ...
                        if (!fileIdList.isEmpty()) {
                                for (Integer fileId : fileIdList) {
                                        diagnosisreportsMapper.updateDiagnosisResultIdByFileId(fileId,
                                                        diagnosisResultsId);
                                }
                        }
                }
